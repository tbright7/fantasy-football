...
const fetchWithRetry = async (
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      clearTimeout(timer);
      return data.data as LeagueDataResponse;
    } catch (error: any) {
...
/**
 * Fetches league data from the `/api/espn/league` endpoint.
 * @param endpoint - The API endpoint to fetch data from. Defaults to `/api/espn/league`.
